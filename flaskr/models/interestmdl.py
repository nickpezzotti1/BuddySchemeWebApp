...
class InterestModel(BasicModel):
    ...
    def delete_interest(self, interest_id, scheme_id=1): # remove default 
        ...
        try:
            self._dao.execute("DELETE FROM Interest WHERE id = %s;", (interest_id, )) #AND scheme_id = %s;", (interest_id, scheme_id))
            self._dao.commit()
    ...
    def insert_interest(self, interest, scheme_id=1):
        ...
        try:
            self._dao.execute("INSERT INTO Interest (interest_name) VALUES(%s);", (interest, )) ##, %s);", (scheme_id, interest))
            self._dao.commit()
